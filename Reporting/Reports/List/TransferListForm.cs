...
{
    public class TransferListForm
    {
        //specific variables
        private List<StockTransfer> dataList = new List<StockTransfer>();
        ...
        public TransferListForm(List<StockTransfer> _dataList, DateTime _startDate, DateTime _endDate, string _printPath, string _resourcePath, Company _company)
        {
            this.invoice = new XmlDocument();
            this.invoice.LoadXml(ParamsRepository.reportXml);
            this.navigator = this.invoice.CreateNavigator();
        ...
        public string generatePdf()
        {
            ...
            // 
            filename = "ListeTransferts-" + Guid.NewGuid().ToString("N").ToUpper() + ".pdf";
        ...
        private Document CreateDocument()
        {
            ...
            this.document.Info.Title = "Liste Transferts";
            this.document.Info.Subject = "Liste Transferts";
        ...
        private void CreatePage()
        {
            ...
            row.Format.Font.Bold = true;
            row.Shading.Color = TableBlue;
            row.Cells[2].AddParagraph("Source");
            row.Cells[2].Format.Alignment = ParagraphAlignment.Center;
            row.Cells[3].AddParagraph("Destination");
            row.Cells[3].Format.Alignment = ParagraphAlignment.Center;

            row.Cells[4].AddParagraph("Vendeur");
        ...
        private void FillContent()
        {
            ...
            if (dataList != null)
            {
                ...
                {
                    StockTransfer transferLine = dataList[i];
                    ...
                    //
                    row1.Cells[1].AddParagraph(transferLine.date.ToString("dd/MM/yyyy HH:mm"));

                    string sourceCompartment = (transferLine.source != null) ? (transferLine.source.name ) : "";
                    row1.Cells[2].AddParagraph(sourceCompartment);

                    string destCompartment = (transferLine.destination != null) ? (transferLine.destination.name) : "";
                    row1.Cells[3].AddParagraph(destCompartment);

                    string sellerName = (transferLine.agent != null) ? (transferLine.agent.user.firstname + " " + transferLine.agent.user.lastname) : "";
            ...
            row.Cells[0].AddParagraph("Nombre total");
