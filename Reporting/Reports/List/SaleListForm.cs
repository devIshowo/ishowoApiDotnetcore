...
namespace ItCommerce.Reporting.Reports
{
    ...
    {
        //specific variables
        private List<ProductSale> dataList = new List<ProductSale>();
        ...
        public SaleListForm(List<ProductSale> _dataList, DateTime _startDate, DateTime _endDate, string _printPath, string _resourcePath, Company _company)
        {
            this.invoice = new XmlDocument();
            this.invoice.LoadXml(ParamsRepository.reportXml);
            this.navigator = this.invoice.CreateNavigator();
        ...
        public string generatePdf()
        {
            ...
            // 
            filename = "ListeVentes-" + Guid.NewGuid().ToString("N").ToUpper() + ".pdf";
        ...
        private Document CreateDocument()
        {
            ...
            this.document.Info.Title = "Liste Ventes";
            this.document.Info.Subject = "Liste Ventes";
        ...
        private void CreatePage()
        {
            ...
            row.Format.Font.Bold = true;
            row.Shading.Color = TableBlue;
            row.Cells[2].AddParagraph("Client");
            row.Cells[2].Format.Alignment = ParagraphAlignment.Center;
            row.Cells[3].AddParagraph("Montant Vente");
        ...
        private void FillContent()
        {
            ...
            if (dataList != null)
            {
                ...
                {
                    ProductSale saleLine = dataList[i];
                    ...
                    //
                    row1.Cells[1].AddParagraph(saleLine.date.ToString("dd/MM/yyyy HH:mm"));

                    string customerName = (saleLine.customer != null) ? (saleLine.customer.nom + " " + saleLine.customer.prenom) : "";                        
                    row1.Cells[2].AddParagraph(customerName);

                    row1.Cells[3].AddParagraph(saleLine.amount_to_pay.ToString(numberFormat, formatProvider) + " FCFA");
                    string sellerName = (saleLine.agent != null) ? (saleLine.agent.user.firstname + " " + saleLine.agent.user.lastname) : "";

                    row1.Cells[4].AddParagraph(sellerName);

                    totalValue += saleLine.amount_to_pay;
