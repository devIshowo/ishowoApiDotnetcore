...
namespace ItCommerce.Reporting.Reports
{
    ...
    public class TransferForm
    {
        ...
        public TransferForm(StockTransfer _transfer, string _printPath, string _resourcePath, Company _company)
        {
            this.invoice = new XmlDocument();
            this.invoice.LoadXml(ParamsRepository.reportXml);
            this.navigator = this.invoice.CreateNavigator();
        ...
        public string generatePdf()
        {
            ...
            // ...
            filename = "Transfert-" + Guid.NewGuid().ToString("N").ToUpper() + ".pdf";
        ...
        private Document CreateDocument()
        {
            ...
            this.document.Info.Title = "Transfert";
            this.document.Info.Subject = "Votre transfert";
        ...
        private void CreatePage()
        {
            ...
            paragraph.AddFormattedText("Transfert du ", TextFormat.Bold);
            paragraph.AddFormattedText(transfer.date.ToString("dd/MM/yyyy HH:mm"), TextFormat.Italic);
            paragraph.Format.Font.Name = "Times New Roman";
            paragraph.Format.Font.Size = 7;
            paragraph.Format.SpaceAfter = 3;

            // Add the print date field and supplier
            paragraph = section.AddParagraph();
            paragraph.Format.SpaceBefore = "2cm";
            paragraph.Format.Alignment = ParagraphAlignment.Left;
            paragraph.Style = "Reference";
            paragraph.AddFormattedText("SOURCE: ", TextFormat.Bold);
            paragraph.AddFormattedText(string.Format("{0}", transfer.source.name), TextFormat.Italic);

            paragraph.AddTab();
            paragraph.AddFormattedText("DESTINATION: ", TextFormat.Bold);
            paragraph.AddFormattedText(string.Format("{0}", transfer.destination.name), TextFormat.Italic);
            ...
            string titleReportText = string.Format("Liste des produits transférés");
        ...
        private void FillContent()
        {
            ...
            if (transfer.product_lines != null)
            {
                #region order lines
                for (int i = 0; i < transfer.product_lines.Count; i++)
                {
                    ProductInStock productLine = transfer.product_lines[i];
                    ...
                    row1.Cells[2].AddParagraph(productLine.quantity_transfer.ToString());

                    row1.Cells[3].AddParagraph(productLine.purchase_price.ToString(numberFormat, formatProvider) + " FCFA"); //ToString("0.00")

                    double extendedPrice = productLine.quantity_transfer * productLine.purchase_price;
            ...
            row.Cells[0].AddParagraph("Nombre de produits");
            row.Cells[0].Format.Font.Bold = true;
            row.Cells[0].Format.Alignment = ParagraphAlignment.Right;
            row.Cells[0].MergeRight = 3;
            row.Cells[4].AddParagraph(transfer.product_lines.Count.ToString(numberFormat, formatProvider) );
