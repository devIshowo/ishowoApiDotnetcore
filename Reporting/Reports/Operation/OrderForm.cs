...
namespace ItCommerce.Reporting.Reports
{
    ...
    public class OrderForm
    {
        ...
        public OrderForm(Order _order, string _printPath, string _resourcePath, Company _company)
        {
            this.invoice = new XmlDocument();
            this.invoice.LoadXml(ParamsRepository.reportXml);
            this.navigator = this.invoice.CreateNavigator();
        ...
        private void CreatePage()
        {
            ...
            paragraph.AddFormattedText(order.date.ToString("dd/MM/yyyy HH:mm"), TextFormat.Italic);
            ...
            paragraph.AddFormattedText(string.Format("{0}", order.supplier.name), TextFormat.Italic);

            paragraph.AddTab();
            paragraph.AddFormattedText("RAYON: ", TextFormat.Bold);
            paragraph.AddFormattedText(string.Format("{0}", order.destination.name), TextFormat.Italic);
        ...
        private void FillContent()
        {
            ...
            if (order.product_lines != null)
            {
                #region order lines
                for (int i = 0; i < order.product_lines.Count; i++)
                {
                    ProductInStock productLine = order.product_lines[i];
                    ...
                    row1.Cells[2].AddParagraph(productLine.quantity.ToString());

                    row1.Cells[3].AddParagraph(productLine.purchase_price.ToString(numberFormat, formatProvider) + " FCFA"); //ToString("0.00")

                    double extendedPrice = productLine.quantity * productLine.purchase_price;
            ...
            row.Cells[4].AddParagraph(order.amount.ToString(numberFormat, formatProvider) + " FCFA");
